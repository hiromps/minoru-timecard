...
- **昼休憩控除**: 所定昼休憩 JST 12:00〜13:00 と実勤務（始業起点に統一後）が重なった分のみ `work_hours` から控除。午前のみ・午後のみ勤務では控除しない。
...
- **残業ルール区分（`employees.overtime_rule_type`、0010で追加）**: `overtime_minutes`の算出方法を社員ごとに切り替える。
