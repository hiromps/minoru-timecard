> ⚠️ **このドキュメントは廃止済み（レガシー）です。**
> 旧構成（Express + SQLite / 社内LAN + IP制限）を前提としており、現行の実態（React SPA + Supabase + Vercel）とは一致しません。
> 歴史的資料として保管しています。現行の正典は [リポジトリ直下の README.md](../../README.md) および [docs/](../) 配下の各ドキュメントを参照してください。
